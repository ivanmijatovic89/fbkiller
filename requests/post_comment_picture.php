...
if(!empty($_POST['id']) && !empty($_POST['comment'])) {
	if(isset($_SESSION['username']) && isset($_SESSION['password']) || isset($_COOKIE['username']) && isset($_COOKIE['password'])) {
		...
		if($verify['username']) {
			...
			if(strlen($_POST['comment']) < $settings['message'] && strlen($_POST['comment']) > 0) {
				$result = $feed->addCommentPhoto($_POST['id'], $_POST['comment']);
				if($result) {
					echo $feed->getLastCommentPhoto();
				} else {
				...
				echo '<div class="message-reply-container" id="post_comment_'.$rand.'"><div class="message-reported">'.sprintf($LNG['comment_too_long'], $settings['message']).'<a onclick="deleteNotification(1, \''.$rand.'\')" title="Delete notification"><div class="delete_btn"></div></a></div></div>';
			}
		}
	}
}
