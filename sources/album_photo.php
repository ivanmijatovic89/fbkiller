...
function PageMain() {
	...
	$skin = new skin('album_photo/rows'); $rows = '';
	
	if(empty($_GET['filter'])) {
		$_GET['filter'] = '';
	}
	// If the message id is not set, or it doesn't consist from digits
	if(!isset($_GET['ap']) || !ctype_digit($_GET['ap'])) {
		header("Location: ".$CONF['url']);
	}
	
	$message = $feed->getAlbumPhoto($_GET['ap']);
	$TMPL['messages'] = $message;
	...
	$TMPL['title'] = 'Album Photo - '.$settings['title'];
